...
for (i in 1:22){
  ...
  GCST2409 <- df_GCST90002409 %>% filter(sprintf("%s", i) == df_GCST90002409$chromosome)
  ...
  ### Making SNP column
  GCST2409$SNP <- paste("chr", GCST2409$chromosome, ":", GCST2409$base_pair_location, sep="")
  ...
  GCST2409_Renamed <- cbind(GCST2409[,c(1,10,5,4,8,6, 7)])
  ### Renaming columns for harmonize_data() function
  colnames(GCST2409_Renamed) <- c("variant_id", "SNP", "other_allele.outcome",  "effect_allele.outcome", "pval.outcome", "beta.outcome", "se.outcome")
